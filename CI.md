...
| [all_green](.github/workflows/all_green_check.yaml) | Linters (PR only), sanity, units, coverage XML, aggregate gate; SonarCloud runs via [`sonarcloud.yml`](.github/workflows/sonarcloud.yml) on **`workflow_run`** | (see jobs) | (see jobs) |
...
SonarCloud analysis runs from **[`sonarcloud.yml`](.github/workflows/sonarcloud.yml)** on **`workflow_run`** after **[`all_green_check.yaml`](.github/workflows/all_green_check.yaml)** succeeds. The **`finalize`** job checks out the triggering commit, downloads **`coverage*`** artifacts from that run, and runs the pinned **SonarSource** scan action with **`sonar.python.coverage.reportPaths`**. Details: [SONARCLOUD.md](SONARCLOUD.md).
