...
class TestDependencyUpdateWithChartRefIsUrl(unittest.TestCase):
    ...
    def test_dependency_update_option_not_defined(self):
        ...
        with patch.object(basic.AnsibleModule, 'run_command') as mock_run_command:
            ...
            with self.assertRaises(AnsibleExitJson) as result:
                helm.main()
        mock_run_command.assert_called_once_with('/usr/bin/helm upgrade -i --reset-values test http://repo.example/charts/application.tgz',
                                                 environ_update={'HELM_NAMESPACE': 'test'})
        assert result.exception.args[0]['command'] == '/usr/bin/helm upgrade -i --reset-values test http://repo.example/charts/application.tgz'
    ...
    def test_dependency_update_option_True_and_replace_option_enabled(self):
        ...
        with patch.object(basic.AnsibleModule, 'run_command') as mock_run_command:
            ...
            with self.assertRaises(AnsibleExitJson) as result:
                helm.main()
        mock_run_command.assert_called_once_with('/usr/bin/helm install --dependency-update --replace test http://repo.example/charts/application.tgz',
                                                 environ_update={'HELM_NAMESPACE': 'test'})
        assert result.exception.args[0]['command'] == '/usr/bin/helm install --dependency-update --replace test http://repo.example/charts/application.tgz'
