...
class TestDependencyUpdateWithoutChartRepoUrlOption(unittest.TestCase):
    ...
    def test_dependency_update_option_not_defined(self):
        ...
        with patch.object(basic.AnsibleModule, 'run_command') as mock_run_command:
            ...
            with self.assertRaises(AnsibleExitJson) as result:
                helm_template.main()
        mock_run_command.assert_called_once_with('/usr/bin/helm template /tmp/path',
                                                 environ_update={})
    ...
    def test_dependency_update_option_false(self):
        ...
        with patch.object(basic.AnsibleModule, 'run_command') as mock_run_command:
            ...
            with self.assertRaises(AnsibleExitJson) as result:
                helm_template.main()
        mock_run_command.assert_called_once_with('/usr/bin/helm --repo=https://charts.com/test template test',
                                                 environ_update={})
        assert result.exception.args[0]['command'] == '/usr/bin/helm --repo=https://charts.com/test template test'

    def test_dependency_update_option_true(self):
        set_module_args({
            'chart_ref': 'https://charts/example.tgz',
        ...
        with patch.object(basic.AnsibleModule, 'run_command') as mock_run_command:
            ...
            with self.assertRaises(AnsibleExitJson) as result:
                helm_template.main()
        mock_run_command.assert_called_once_with('/usr/bin/helm template https://charts/example.tgz --dependency-update',
                                                 environ_update={})
        assert result.exception.args[0]['command'] == '/usr/bin/helm template https://charts/example.tgz --dependency-update'
