...
class TestK8sTurboMode:
    ...
    def test_turbo_mode_disabled_on_newer_ansible(self, monkeypatch, ansible_version):
        ...
        with patch.dict(
            "sys.modules",
            {
                "ansible.module_utils.ansible_release": MagicMock(
            ...
            assert (
                "ENABLE_TURBO_MODE is deprecated"
                in mock_display.deprecated.call_args[0][0]
            )

            mock_display.warning.assert_called()
            assert (
                f"ENABLE_TURBO_MODE is ignored on ansible-core {ansible_version}"
                in mock_display.warning.call_args[0][0]
            )
